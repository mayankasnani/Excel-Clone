...
$(document).ready(function () {
    ...
    // This is done to fill alphabets in column name
    for (i = 0; i < 100; i++) {
        let n = i;
        let ans = " ";
        while (n > 0) {
            ...
            column = $(`<div class="column-name colcod-${ans} colid-${i}" id="colcod-${ans}"> ${ans} </div>`)
            $(".column-name-container").append(column);


        }

        row = (`<div class="row-name" id ="${i}">${i}</div>`)
        $(".row-name-container").append(row);
    }

    // This is done for creating Cells 
    for (let i = 1; i < 100; i++) {
        ...
        for (let j = 1; j < 100; j++) {
            ...
            let column = $(`<div class="cell-name"  id="row-${i}-col-${j}" data="${colcode}"></div>`);
            row.append(column);
    ...
    //To remove or add background color while clicking on any align icon
    //By default left-align is selected 
    ...
    //To remove or add background color while clicking on any style icon
    //No icon is selected by Default
    $(".style-icon").click(function () {
        $(this).toggleClass("selected");
    }
    )

    //To remove or add border color while clicking on any cell and 
    //multiple select

     $(".cell-name").click(function (e) {
        if(e.ctrlKey) {
            ...
            $(".cell-name.selected").removeClass("selected");
        }
        $(this).addClass("selected");
        changeHeader(this);
    });

     //For displaying properties of the selected cell (Two way linking)
    ...
    //To add text after double click in a cell and focus
    $(".cell-name").dblclick(function () {
        $(".cell-name.selected").removeClass("selected");
        $(this).addClass("selected");
    ...
    //TO update text written in the class for cell data
    //To make contenteditable false
    $(".cell-name").blur(function()
    {
        $("cell-name-selected").attr("contenteditable","false");
        console.log($(this).text);
        updateCell("text",$(this).text());
    })

    //to add scroll for input-cell-container and 
    //column-container togethor
...
//Function to get RowId and colId
...
//UpdateCell is used to store data in objects created for sheets and delete 
//the particular (row and column) if it has default properties
//each is used for iteration in jss

    function updateCell(property,value,defaultPossible) {
        $(".cell-name.selected").each(function() {
            ...
            if(cellData[selectedSheet][rowId]) { 
                if(cellData[selectedSheet][rowId][colId]) {
                    ...
                    cellData[selectedSheet][rowId][colId] = {...defaultProperties}; //assigning sparsely the default properties
                    cellData[selectedSheet][rowId][colId][property] = value;
                }
            } else {
    ...
    //click on the icon also to pick up color for bg and text
    $(".fill-color").click(function(){
        $(".background-color-picker").click();
    });
   
    $(".fill-text").click(function(){
        $(".text-color-picker").click();
    });


    
    //update cell with color chosen for test and background color
    ...
    $(".sheet-icon-add").click(function(){
        ...
        $(".sheet-bar").append(`<div class="sheet-number sheet-tab selected">${sheetName}</div>`);
        addSheetEvents();
    });
